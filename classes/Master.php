...
class Master extends DBConnection
{
  ...
  function save_supplier()
  {
    ...
    if ($check > 0) {
      $resp['status'] = 'failed';
      $resp['msg'] = "le nom du fournisseur existe déjà.";
      return json_encode($resp);
      exit;
    }
    if (empty($id)) {
    ...
    if ($save) {
      $resp['status'] = 'success';
      if (empty($id)) {
        $res['msg'] = "Nouveau fournisseur enregistré avec succès.";
        $id = $this->conn->insert_id;
      } else {
        $res['msg'] = "Mise à jour du fournisseur réussie.";
      }
  ...
  function save_item()
  {
    ...
    if ($check > 0) {
      $resp['status'] = 'failed';
      $resp['msg'] = "L'article existe déjà chez le fournisseur sélectionné.";
      return json_encode($resp);
      exit;
    }
    if (empty($id)) {
    ...
    if ($save) {
      $resp['status'] = 'success';
      if (empty($id))
        $this->settings->set_flashdata('success', "Nouvel élément sauvegardé avec succès.");
      else
        $this->settings->set_flashdata('success', "L'article a été mis à jour avec succès.");
    } else {
      $resp['status'] = 'failed';
      $resp['err'] = $this->conn->error . "[{$sql}]";
    }
  ...
  function delete_item()
  {
    ...
    if ($del) {
      $resp['status'] = 'success';
      $this->settings->set_flashdata('success', "L'élément a été supprimé avec succès.");
    } else {
      $resp['status'] = 'failed';
      $resp['error'] = $this->conn->error;
    }
  ...
  function save_po()
  {
    ...
    if ($resp['status'] == 'success') {
      if (empty($id)) {
        $this->settings->set_flashdata('success', " La nouvelle commande a été créée avec succès.");
      } else {
        $this->settings->set_flashdata('success', " Mise à jour réussie des détails du bon de commande.");
      }
    }
  ...
  function delete_po()
  {
    ...
    if ($del) {
      $resp['status'] = 'success';
      $this->settings->set_flashdata('success', "Détails de la commande Supprimé avec succès.");
      if ($bo->num_rows > 0) {
  ...
  function save_receiving()
  {
    ...
    if ($resp['status'] == 'success') {
      if (empty($id)) {
        $this->settings->set_flashdata('success', " Le nouveau stock a été reçu avec succès.");
      } else {
        $this->settings->set_flashdata('success', " Mise à jour réussie des détails du stock reçu.");
      }
    }
  ...
  function delete_receiving()
  {
    ...
    if ($del) {
      $resp['status'] = 'success';
      $this->settings->set_flashdata('success', "Détails de l'ordre reçu supprimés avec succès.");
  ...
  function save_return()
  {
    ...
    if ($resp['status'] == 'success') {
      if (empty($id)) {
        $this->settings->set_flashdata('success', " Un nouvel enregistrement d'article retourné a été créé avec succès.");
      } else {
        $this->settings->set_flashdata('success', " Mise à jour réussie des enregistrements d'articles retournés.");
      }
    }
  ...
  function delete_return()
  {
    ...
    if ($del) {
      $resp['status'] = 'success';
      $this->settings->set_flashdata('success', "Suppression réussie des enregistrements d'articles retournés.");
      if (isset($res)) {
        $this->conn->query("DELETE FROM `stock_list` where id in ({$res['stock_ids']})");
      }
    } else {
  ...
  function save_sale()
  {
    ...
    if ($resp['status'] == 'success') {
      if (empty($id)) {
        $this->settings->set_flashdata('success', " Un nouveau enregistrement de vente a été créé avec succès.");
      } else {
        $this->settings->set_flashdata('success', " L'enregistrement des ventes a été mis à jour avec succès.");
      }
    }
  ...
  function delete_sale()
  {
    ...
    if ($del) {
      $resp['status'] = 'success';
      $this->settings->set_flashdata('success', "Enregistrement de vente supprimé avec succès.");
      if (isset($res)) {
        $this->conn->query("DELETE FROM `stock_list` where id in ({$res['stock_ids']})");
      }
    } else {
